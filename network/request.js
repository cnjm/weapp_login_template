import { DOMAIN,TIMEOUT, CONTENT_TYPE, HTTP_STATUS, DELAYED, Authorization, resCode} from "./config";
...
export const request = (params)=>{
  return new Promise((resolve,reject)=>{
    ...
    if(identity && !token){ //首次进入需要认证并且当前无登录状态时延时请求,默认200毫秒等待
      setTimeout(() => {
    ...
    wx.request({
      ...
      success (res) {
        ...
        if ((Array.isArray(HTTP_STATUS.SUCCESS) && HTTP_STATUS.SUCCESS.indexOf(code)!=-1) || code === HTTP_STATUS.SUCCESS) {
          resolve(res.data);
          return;
        } else if (code === HTTP_STATUS.AUTHENTICATE) { //当提示token失效时
          const app = getApp();
          const LOGIN_SWITCH = app.globalData.LOGIN_SWITCH;// login锁，确保没有登录冲突
          if(LOGIN_SWITCH){ //如果已锁说明有 在重新登录 只需要等待一下 继续请求
            setTimeout(() => {
              resolve(request(params));
            }, DELAYED);
          }else{
